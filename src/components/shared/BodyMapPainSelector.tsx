...
/**
 * Reusable interactive body-map pain selector — single source of truth for
 * the SVG body map used in Triage Wizard Step 1, the Daily Check-In modal,
 * and any read-only doctor-side pain map snapshots.
 *
 * Modes:
 *   - default: full interactive (tap regions to open inline detail panel)
 *   - readOnly: render coloured regions + numeric overlays only (doctor view,
 *     dashboard snapshot)
 *   - showDuration={false}: hide the "How long?" dropdown — daily check-ins
 *     don't need duration since it's implicit in daily tracking.
 */
...
export interface PainRegion {
  ...
  duration?: string;
  radiatesTo?: string;
  /** Optional simple yes/no for "does this radiate?" used in the check-in
   *  flow where the patient picks a target region only if true. */
  radiates?: boolean;
}

interface BodyMapPainSelectorProps {
  initialPainRegions?: PainRegion[];
  onChange?: (regions: PainRegion[]) => void;
  readOnly?: boolean;
  /** Whether to show the "How long?" duration dropdown in the inline detail
   *  panel. Daily check-ins pass false; triage/onboarding leave default. */
...
export function BodyMapPainSelector({
  initialPainRegions = [],
  onChange,
  readOnly = false,
  showDuration = true,
}: BodyMapPainSelectorProps) {
  const [view, setView] = useState<'front' | 'back'>('front');
  const [activeRegion, setActiveRegion] = useState<string | null>(null);
  const detailRef = useRef<HTMLDivElement>(null);

  const regions = initialPainRegions;
  ...
  const updateRegion = useCallback((regionId: string, regionLabel: string, updates: Partial<PainRegion>) => {
    ...
    if (existing) {
      ...
      updated = [...regions, {
        ...
        intensity: 3,
        characters: [],
        duration: showDuration ? 'Days' : undefined,
        ...updates
      }];
    }
    onChange?.(updated);
  }, [regions, onChange, showDuration]);
  ...
  }, [activeRegion, regions, updateRegion, readOnly]);
  ...
  return (
    <div className="space-y-3">
      <div className="relative rounded-2xl border border-border/60 bg-gradient-to-b from-card to-secondary/20 shadow-sm overflow-hidden">
                ...
                >
                  ...
                  <path
                    ...
                    className={cn(
                      ...
                      !painData && !readOnly && "hover:fill-primary/10"
                    )}
                  />
        ...
        {!readOnly && activeRegion && activeRegionDef && (
        ...
        <div className="flex flex-wrap gap-1.5">
          {regions.map(r => (
            <button
              type="button"
              key={r.regionId}
              onClick={() => !readOnly && setActiveRegion(activeRegion === r.regionId ? null : r.regionId)}
              className={cn(
                "group inline-flex items-center gap-1 sm:gap-1.5 px-2 sm:px-2.5 py-1 rounded-full text-[10px] sm:text-[11px] font-bold border transition-all duration-200",
...
function PainDetailInline({ region, painData, allRegions, onUpdate, onRemove, onClose, showDuration }: PainDetailInlineProps) {
  ...
  const radiates = painData?.radiates ?? !!radiatesTo;
  const [showRadiation, setShowRadiation] = useState(radiates);
  ...
  return (
    <div className="bg-card/80 backdrop-blur-sm">
      ...
      <div className="px-3 pb-3 sm:px-4 sm:pb-4 space-y-3 sm:space-y-4">
        ...
        <div className={cn("grid gap-3", showDuration ? "grid-cols-1 sm:grid-cols-2" : "grid-cols-1")}>
          {showDuration && (
            <div className="space-y-1.5">
          ...
          <div className="space-y-1.5">
            <div className="flex items-center gap-2">
              <Switch
                ...
                onCheckedChange={(checked) => {
                  ...
                  onUpdate({
                    radiates: checked,
                    radiatesTo: checked ? radiatesTo : undefined,
                  });
                }}
                className="scale-90"
              />
            ...
            {showRadiation && (
              <Select value={radiatesTo || ''} onValueChange={(val) => onUpdate({ radiatesTo: val, radiates: true })}>
                <SelectTrigger className="h-8 text-xs rounded-lg">
