...
import { RecipeLibraryContent } from "./RecipeLibrary";
import type {
  AyurvedicFood,
  FoodCategory,
  FoodDatabaseStats,
...
/**
 * /food-database — Ayurvedic Food Database + Recipe Library, in one place.
 *
 ...
 *
 * The page renders two tabs: "Food Database" and "Recipe Library". The
 * recipe tab embeds the existing RecipeLibraryContent so we don't
 * duplicate any logic. The legacy /recipe-library route redirects here
 * with ?tab=recipes pre-selected.
 */
export default function FoodDatabase() {
  // Read the initial tab from the URL so deep links land on the correct
  // panel. The Tabs widget below is uncontrolled after first render.
  const initialTab = (() => {
    const t = new URLSearchParams(window.location.search).get("tab");
    return t === "recipes" ? "recipes" : "foods";
  })();
  const [tab, setTab] = useState<"foods" | "recipes">(initialTab);

  // Keep the URL ?tab= param in sync so a refresh / bookmark preserves
  // whichever tab the user last selected.
  useEffect(() => {
  ...
  return (
    <AppLayout>
      <div className="container max-w-7xl mx-auto px-4 py-6">
        <Tabs value={tab} onValueChange={(v) => setTab(v as "foods" | "recipes")}>
...
/**
 * FoodDatabaseContent — the original food-management UI without AppLayout
 * so it can be embedded in the combined tabbed page.
 */
function FoodDatabaseContent() {
  const { role } = useAuth();
