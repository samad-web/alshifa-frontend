...
export default function WorkflowAutomation() {
    ...
    const evaluateMutation = useMutation({
        ...
        onError: (err) => {
            // Surface a contextual toast (no raw error dialog). A bare
            // "Forbidden" message from the API confused admin doctors who
            // were authorised — prefix it so the cause is obvious.
            const reason = err instanceof Error ? err.message : "Unknown error";
            toast.error(`Could not trigger evaluation: ${reason}`);
        },
    });
...
function RuleBuilderDialog({
    ...
    const saveMutation = useMutation({
        ...
        onSuccess: () => {
            // The backend now fires the engine once on create (scoped to
            // this rule), so eligible patients get the message within
            // seconds — copy the toast to set that expectation.
            toast.success(isEdit ? "Rule updated" : "Rule created — actions will fire shortly");
            onSaved();
        },
        onError: (err) => {
